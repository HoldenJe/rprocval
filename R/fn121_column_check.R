...
#' Check FN121 Columns based on GL Template 5
#'
#' @param FN121 FN121 table that adheres to the GL Template 5 data model
#'
#' @return
#' @export
#'
...
fn121_column_check_t5 <- function(FN121) {
  req_names <-
    c("PRJ_CD",
      "SAM",
      "EFFDT0",
  ...
  if (!("SIDEP0" %in% names(FN121))) {
    usethis::ui_oops("SIDEP0 is required in FN121")
  }
  if (!("SITEM0" %in% names(FN121))) {
    usethis::ui_oops("SITEM0 is required in FN121")
  }
  if (!("EFFDUR" %in% names(FN121))) {
    usethis::ui_oops("EFFDUR is required in FN121")
  }
