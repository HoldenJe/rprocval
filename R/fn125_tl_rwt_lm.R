...
fn125_tl_rwt_lm <- function(FN125, makeplot = F, fail_criteria = 0.3) {
  ...
  if(length(!(is.na(FN125$RWT))) < 10 | length(!(is.na(FN125$TLEN))) < 10){
    usethis::ui_oops(paste0("Fewer than 10 records exists for SPC == ", unique(FN125$SPC)))
    return(FN125)
  } else {
    TLEN.RWT <- nls(RWT~alpha*TLEN^(beta), data=FN125, start=list(alpha=0.000005, beta=3))
    FN125 <- FN125 |>
    ...
    if(sum(FN125$qid4_error, na.rm = T) == 0){
      done_message <- paste0("No instances of irregular TLEN~RWT values found for SPC == ", unique(FN125$SPC))
    ...
    if(is.na(sum(FN125$qid4_error))){usethis::ui_warn("NA values in TLEN or RWT")}
